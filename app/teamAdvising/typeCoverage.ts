...
import {
  TypeChart,
  TypeChartByGeneration,
  ITypeChartEntry,
} from "../data/typeChart";

export function CalculateIndividualCoverages({
  PokemonGame,
  PokemonTeam,
}: {
  PokemonGame: string;
  ...
  const gameTypeChart: TypeChart = GetTypeChartByGeneration({ PokemonGame });

  // First we need to calculate each individual Pokemons defensive coverage.
  const coverages = PokemonTeam.reduce<Array<ITypeChartEntry>>(
    (acc, pokemon) => {
      let coverage: ITypeChartEntry = {
      ...
      coverage.weak = new Set([
        ...[...weak1].filter(
          (t) => !resists2.has(t) && !coverage.immune.has(t),
        ),
        ...[...weak2].filter(
          (t) => !resists1.has(t) && !coverage.immune.has(t),
        ),
      ]);
      coverage.resists = new Set([
        ...[...resists1].filter(
          (t) => !weak2.has(t) && !coverage.immune.has(t),
        ),
        ...[...resists2].filter(
          (t) => !weak1.has(t) && !coverage.immune.has(t),
        ),
      ]);
...
export function GetTypeChartByGeneration({
  PokemonGame,
}: {
  PokemonGame: string;
}): TypeChart {
  ...
  const gameTypeChartGeneration =
    Object.keys(TypeChartByGeneration)
      ...
      .sort((a, b) => b - a)[0] ?? 100;
  return TypeChartByGeneration[gameTypeChartGeneration];
}
